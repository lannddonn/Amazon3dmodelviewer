...
$output = '';
if (isset($_POST['url'])) {
    if (parse_url($_POST['url'], PHP_URL_HOST) == 'www.amazon.com') {
        ...
        if ($result && preg_match('@href="(/view-3d[^"]+)"@', $result, $matches)) {
            $output = '3D Model: <a target="_blank" href="https://www.amazon.com'.$matches[1].'">https://www.amazon.com'.$matches[1].'</a> (The zip file with the model can be found in the network tab in Developer tools)';
        } else {
            $output = 'Check for AR View Zip instead!';
        }

        curl_close($ch);
    } else {
        $output = 'Could not parse URL. Make sure it starts with https://www.amazon.com/';
    }
}
